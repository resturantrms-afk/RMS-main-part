            ...
            builder: (context, profileState) {
              ...
              if (profileState is MenuProfilesLoaded) {
                final activeProfiles =
                    profileState.profiles.where((p) => p.isActive).toList();
                if (activeProfiles.isNotEmpty) {
                  final Set<String> activeIds = {};
                  for (var p in activeProfiles) {
                    activeIds.addAll(p.menuItemIds);
                  }
                  allowedMenuItems = allMenuItems
                      .where((i) => activeIds.contains(i.id))
                      .toList();
                }
              }

              final int availableCount = allowedMenuItems
                  .where((i) => i.status == MenuItemStatus.available)
                  .length;
              final int unavailableCount = allowedMenuItems
                  .where((i) => i.status == MenuItemStatus.unavailable)
                  .length;

              List<String> allCategories =
                  _extractUniqueCategories(allowedMenuItems);
              ...
              return ValueListenableBuilder<TextEditingValue>(
                ...
                builder: (context, textEditingValue, child) {
                  ...
                  if (_availabilityTabIndex == 0) {
                    ...
                    filteredItems = filteredItems
                        .where(
                            (item) => item.status == MenuItemStatus.unavailable)
                        .toList();
                  }
