...
import 'package:rmss/features/admin/views/desktop/menu%20widgets/menu_profile_dialog.dart';
import 'package:rmss/features/admin/views/desktop/pages/menu_profile_details_page.dart';
                    ...
                    builder: (context, profileState) {
                      if (profileState is MenuProfilesLoaded) {
                        ...
                        if (profiles.isEmpty) {
                          return Center(
                            child: Text(
                              'No Menu Profiles Found. Create one!',
                              style: TextStyle(
                                fontSize: 18,
...
class _CreateMenuProfileButton extends StatefulWidget {
  @override
  State<_CreateMenuProfileButton> createState() =>
      _CreateMenuProfileButtonState();
}

class _CreateMenuProfileButtonState extends State<_CreateMenuProfileButton> {
  bool _hovered = false;
      ...
      child: AnimatedScale(
        ...
        child: GestureDetector(
          onTap: () {
            showDialog(
              context: context,
              builder: (context) => const MenuProfileDialog(),
            );
          },
          child: Container(
...
class _MenuProfileCard extends StatelessWidget {
  ...
  Future<void> _confirmDelete(
    ...
    MenuProfileModel profile,
  ) async {
    final bool? confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Menu Profile'),
    ...
    if (confirm == true && context.mounted) {
      context.read<MenuProfileBloc>().add(DeleteMenuProfile(profile: profile));
    }
  }
}
